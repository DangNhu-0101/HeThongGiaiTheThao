...
export const register = async (req, res) => {
    try {
        const { username, password, email, role, confirmPassword,phoneNumber } = req.body;

        // 1. Validate input
        if (!username || !password || !email || !confirmPassword || !role || !phoneNumber) {
            return res.status(400).json({
                message: "Vui lòng nhập đầy đủ thông tin (Tên đăng nhập, Mật khẩu, Email, Tên hiển thị, số điện thoại)"
            });
        }
