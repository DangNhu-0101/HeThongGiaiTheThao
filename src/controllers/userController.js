...
export const completeUser = async (req, res) => {
    try {
        ...
        switch (user.role) {
            ...
            case 'referee': {
                const { name,  experienceYears, birthDay, gender } = req.body;
                if (!name || !birthDay || !experienceYears) {
                    return res.status(400).json({ message: "Tên, SĐT, Số năm kinh nghiệm và Ngày sinh Trọng tài không được để trống" });
                }
            ...
            case 'org': {
                const { orgName, phone, address } = req.body;
