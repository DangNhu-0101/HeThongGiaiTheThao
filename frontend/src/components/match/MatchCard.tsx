...
export const MatchCard: React.FC<MatchCardProps> = ({ match, variant, onEdit }) => {
  ...
  return (
    <Card className={cn(
      ...
      isGroup
        ? "bg-gradient-to-br from-blue-50 to-indigo-50 border-l-blue-500"
        : "bg-gradient-to-br from-violet-50 to-fuchsia-50 border-l-violet-500"
    )}>
      <CardContent className="p-4">
        ...
        <div className="mb-4 space-y-2">
          <div className="grid grid-cols-[1fr_auto_1fr] items-center gap-2 text-sm font-semibold text-slate-700">
            <span className={cn("truncate", !match.teamA && "italic text-slate-400")}>
              {match.teamA?.name || 'Chưa xác định'}
            </span>
            <span className="text-xs text-slate-400">VS</span>
            <span className={cn("truncate text-right", !match.teamB && "italic text-slate-400")}>
              {match.teamB?.name || 'Chưa xác định'}
            </span>
          </div>
        </div>
