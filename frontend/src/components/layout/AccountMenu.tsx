...
import { ChevronDown, LayoutDashboard, LogOut, UserRound, UsersRound } from "lucide-react";
import { canManageTournaments } from "@/libs/auth";
...
const readDisplayName = (user: ReturnType<typeof useAuthStore.getState>["user"]) => {
  ...
  return organizationName || user?.profile?.name || user?.playerProfile?.name || user?.player?.name || user?.username || "Tài khoản";
};
  ...
  if (!isLoggedIn) {
    return (
      <div className="flex items-center gap-2">
        <Link
          to="/login"
          className={`inline-flex h-10 items-center justify-center rounded-md px-4 py-2 text-sm font-medium transition-colors focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-ring/25 ${
        ...
        <Link
          to="/register"
          className="inline-flex h-10 items-center justify-center rounded-md bg-primary px-4 py-2 text-sm font-medium text-primary-foreground transition-colors hover:bg-primary-hover focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-primary/25"
        >
          Đăng ký
        </Link>
      </div>
    ...
    <div className="relative">
      ...
      >
        <div className={`text-right ${compact ? "hidden lg:block" : "hidden sm:block"}`}>
          ...
          <p className={`max-w-32 truncate text-[11px] ${dark ? "text-white/60" : "text-muted-foreground"}`}>{contact}</p>
        </div>
        <div className="flex size-9 items-center justify-center rounded-full border border-white/25 bg-white/12 text-xs font-bold text-white">
          {avatarText}
        </div>
        <ChevronDown className={`size-4 transition-transform ${open ? "rotate-180" : ""} ${dark ? "text-white/70" : "text-muted-foreground"}`} />
      </button>

      {open && (
        <div
          role="menu"
          className="absolute right-0 z-50 mt-2 w-[min(18rem,calc(100vw-2rem))] overflow-hidden rounded-2xl border border-border bg-card text-foreground shadow-[var(--shadow-panel)]"
        >
          ...
          <button
            ...
            role="menuitem"
            className="flex w-full items-center gap-3 px-4 py-3 text-left text-sm text-primary transition-colors hover:bg-red-50 focus-visible:outline-none focus-visible:bg-red-50"
          >
