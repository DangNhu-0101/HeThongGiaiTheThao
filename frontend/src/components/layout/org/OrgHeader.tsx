...
const OrgHeader = ({ setSidebarOpen }: { setSidebarOpen: (v: boolean) => void }) => {
  return (
    <header className="h-16 border-b border-border bg-card flex items-center justify-between px-4 lg:px-8 shrink-0 sticky top-0 z-30 shadow-sm">
      <div className="flex items-center gap-4">
        ...
        <div className="hidden md:flex items-center relative">
          ...
          <input
            type="text"
            placeholder="Tim kiem giai dau, doi thi..."
            className="pl-9 pr-4 py-2 text-sm bg-muted/50 border border-border rounded-lg w-64 xl:w-80 focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary transition-all"
          />
        </div>
      </div>
