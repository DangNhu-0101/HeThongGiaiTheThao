...
import { Activity, Building2, FileBarChart, LayoutDashboard, LogOut, Newspaper, ServerCog, Settings, Users, X } from "lucide-react";
...
const NAV_ITEMS = [
  ...
  { group: "Quản lý nền tảng", items: [
    { name: "Tổ chức", icon: Building2, path: "/admin/organizations" },
  ...
  { group: "Hệ thống", items: [
    { name: "Tính năng Backend", icon: ServerCog, path: "/admin/backend-features" },
        ...
        <div className="shrink-0 border-b border-white/10 bg-header p-4 text-white">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              <div className="flex h-8 w-8 items-center justify-center rounded-md bg-primary font-extrabold text-white shadow-sm">SA</div>
              <div>
                <h3 className="text-sm font-bold leading-tight">System Admin</h3>
            ...
            <div key={group.group}>
              ...
              <ul className="space-y-1">
                {group.items.map((item) => {
                  ...
                  return (
                    <li key={item.path}>
                      <Link to={item.path} onClick={() => setOpen(false)} className={`flex items-center rounded-lg px-3 py-2.5 text-sm font-medium transition-all ${isActive ? "bg-primary font-bold text-primary-foreground shadow-sm" : "text-muted-foreground hover:bg-secondary hover:text-secondary-foreground"}`}>
