...
interface Props { records: OrgVenueRecord[]; isMobile: boolean; }

const VenueMgmtTable = ({ records, isMobile }: Props) => {
              ...
              <tr key={r.id} className="hover:bg-muted/10 transition-colors group">
                ...
                <td className="p-4 text-center">
                  <div className="flex items-center justify-center gap-1 opacity-50 group-hover:opacity-100 transition-opacity">
                    ...
                    <button className="p-1.5 text-red-500 hover:bg-red-50 rounded transition-colors" title="Xóa"><Trash2 className="w-4 h-4" /></button>
                  </div>
                </td>
              </tr>
            ))}
