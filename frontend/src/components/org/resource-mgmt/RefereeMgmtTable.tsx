...
interface Props { records: OrgRefereeRecord[]; isMobile: boolean; }

const RefereeMgmtTable = ({ records, isMobile }: Props) => {
  const renderStatus = (status: string) => {
    switch (status) {
      ...
      case 'Assigned': return <span className="bg-blue-50 text-blue-600 border border-blue-200 text-[10px] font-bold px-2 py-0.5 rounded w-max">Đang làm nhiệm vụ</span>;
                ...
                <td className="p-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-[10px] text-muted-foreground mt-0.5">ID: {r.refId} • {r.experience} năm kinh nghiệm</p>
                    </div>
                  </div>
                </td>
