...
const emptyStats = (): TeamMgmtStat[] => [
  { id: "total", label: "Tổng đội", value: 0, iconType: "total", color: "text-blue-600 bg-blue-100" },
  { id: "approved", label: "Đã duyệt", value: 0, iconType: "approved", color: "text-green-600 bg-green-100" },
  { id: "pending", label: "Chờ duyệt", value: 0, iconType: "pending", color: "text-orange-600 bg-orange-100" },
  { id: "rejected", label: "Từ chối", value: 0, iconType: "rejected", color: "text-red-600 bg-red-100" },
  { id: "athletes", label: "Tổng VĐV", value: 0, iconType: "athletes", color: "text-purple-600 bg-purple-100" },
  { id: "sports", label: "Môn thi", value: 0, iconType: "sports", color: "text-gray-600 bg-gray-100" },
  { id: "free", label: "Đội miễn phí", value: 0, iconType: "free", color: "text-indigo-600 bg-indigo-100" },
];
    ...
    try {
      ...
      const records: OrgTeamRecord[] = participantsResponse.data.data
        ...
        .map((item) => ({
          ...
          avatars: item.lineup.map((lineup, memberIndex) => {
            const player = typeof lineup.Player === "string" ? undefined : lineup.Player;
            return player?.avatar || player?.name?.slice(0, 1) || `${memberIndex + 1}`;
          }),
        }));
      const athleteCount = records.reduce((sum, item) => sum + item.playersCount, 0);
      const stats: TeamMgmtStat[] = [
        { id: "total", label: "Tổng đội", value: records.length, iconType: "total", color: "text-blue-600 bg-blue-100" },
        { id: "approved", label: "Đã duyệt", value: records.filter((item) => item.status === "Approved").length, iconType: "approved", color: "text-green-600 bg-green-100" },
        { id: "pending", label: "Chờ duyệt", value: records.filter((item) => item.status === "Pending").length, iconType: "pending", color: "text-orange-600 bg-orange-100" },
        { id: "rejected", label: "Từ chối", value: records.filter((item) => item.status === "Rejected").length, iconType: "rejected", color: "text-red-600 bg-red-100" },
        { id: "athletes", label: "Tổng VĐV", value: athleteCount, iconType: "athletes", color: "text-purple-600 bg-purple-100" },
        { id: "sports", label: "Môn thi", value: tournament.sportType ? 1 : 0, iconType: "sports", color: "text-gray-600 bg-gray-100" },
        { id: "free", label: "Đội miễn phí", value: records.filter((item) => item.isFree).length, iconType: "free", color: "text-indigo-600 bg-indigo-100" },
      ];
