...
const TournamentDetailView = () => {
  ...
  // Load organizations + tournament data
  useEffect(() => {
  ...
  if (loading) return <div className="flex justify-center items-center min-h-screen bg-slate-100 text-cyan-600 font-bold text-xl">ĐANG TẢI DỮ LIỆU...</div>;
  if (!tournament) return <div className="flex justify-center items-center min-h-screen bg-slate-100 text-red-500 font-bold text-xl">KHÔNG TÌM THẤY GIẢI ĐẤU</div>;
    ...
    <>
      <style>{`
        ...
        @media (max-width: 640px) {
          .tm-dialog { max-width: 95%; border-radius: 16px; }
        }
        ...
        @media (max-width: 640px) {
          .tm-header { padding: 16px 20px; }
          .tm-header h2 { font-size: 18px; }
        }
        ...
        @media (max-width: 640px) {
          .tm-section { padding: 14px; border-radius: 16px; }
        }

        .tm-label-sec {
          font-size: 11px; font-weight: 800; color: #018ABE;
          text-transform: uppercase; letter-spacing: 1.5px; margin-bottom: 15px;
          display: flex; align-items: center; gap: 8px;
        }
        .tm-label-sec::before {
          content: ""; width: 4px; height: 14px; background: #018ABE; border-radius: 4px;
        }

        .tm-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 15px; }
        @media (max-width: 768px) { .tm-grid { grid-template-columns: 1fr; gap: 12px; } }
        ...
        @media (max-width: 640px) {
          .tm-input, .tm-select, .tm-textarea { padding: 10px; font-size: 16px; }
        }
        .tm-input:focus, .tm-select:focus, .tm-textarea:focus {
          border-color: #018ABE; box-shadow: 0 0 0 4px rgba(1,138,190,0.08);
        }

        .tm-sport-tag {
          padding: 8px 18px; border-radius: 12px; border: 1.5px solid #E2E8F0;
          cursor: pointer; font-size: 13px; font-weight: 600;
          background: #fff; color: #64748b; transition: all 0.2s;
        }
        @media (max-width: 640px) {
          .tm-sport-tag { padding: 10px 16px; font-size: 14px; }
        }
        .tm-sport-tag.sel { background: #018ABE; color: #fff; border-color: #018ABE; }

        .tm-cat-card {
          border: 1px solid #EEF6FB; border-radius: 16px;
          padding: 18px; background: #F8FAFC; margin-bottom: 10px;
        }
        @media (max-width: 640px) { .tm-cat-card { padding: 12px; } }

        .tm-upload-box {
          border: 2px dashed #CBD5E1; border-radius: 16px; height: 110px;
          display: flex; align-items: center; justify-content: center;
          position: relative; overflow: hidden; cursor: pointer; background: #F1F5F9;
        }
        @media (max-width: 640px) { .tm-upload-box { height: 90px; } }
        ...
        @media (max-width: 640px) { .tm-footer { padding: 16px 20px; flex-direction: column; } }

        .tm-btn-submit {
          flex: 1; background: #018ABE; color: #fff; border: none;
          padding: 14px; border-radius: 14px; font-weight: 700; cursor: pointer; transition: 0.2s;
        }
        .tm-btn-submit:hover { background: #02457A; }

        .tm-cancel-btn {
          padding: 12px 25px; border-radius: 14px; border: 1.5px solid #E2E8F0;
          background: #fff; cursor: pointer; font-weight: 600; color: #64748b;
        }
        ...
        .tm-status-upcoming { background: #E0F2FE; color: #0369A1; }
        .tm-status-actived { background: #DCFCE7; color: #166534; }
        .tm-status-completed { background: #F3F4F6; color: #6B7280; }
      `}</style>
          ...
          <div className="tm-header">
            <div>
              ...
              <div style={{ marginTop: 4 }}>
                <span className={`tm-status-badge tm-status-${tournament.status}`}>
                  {tournament.status}
                </span>
            ...
            <form id="tour-form" onSubmit={handleSubmit}>
              ...
              <div className="tm-section">
                ...
                <div className="tm-grid">
                  ...
                  <div><input name="slogan" className="tm-input" value={formData.slogan} onChange={handleTextChange} placeholder="Slogan" /></div>
                  <div>
                  ...
                  <div className="tm-full">
                    <textarea name="description" className="tm-textarea" value={formData.description} onChange={handleTextChange} placeholder="Mô tả giải đấu..." />
                  </div>
                </div>
              </div>
              ...
              <div className="tm-section">
                ...
                <div className="tm-grid">
                  <div><input name="name" className="tm-input" value={contactPerson.name} onChange={handleContactChange} placeholder="Người phụ trách" /></div>
                  <div><input name="phone" className="tm-input" value={contactPerson.phone} onChange={handleContactChange} placeholder="Hotline" /></div>
                  <div><label style={{fontSize:9, fontWeight:700, color:'#94a3b8'}}>MỞ ĐĂNG KÝ</label><input type="datetime-local" name="registrationStart" className="tm-input" required value={timeLine.registrationStart} onChange={handleTimeChange} /></div>
                  <div><label style={{fontSize:9, fontWeight:700, color:'#94a3b8'}}>ĐÓNG ĐĂNG KÝ</label><input type="datetime-local" name="registrationEnd" className="tm-input" required value={timeLine.registrationEnd} onChange={handleTimeChange} /></div>
                  <div><label style={{fontSize:9, fontWeight:700, color:'#94a3b8'}}>KHAI MẠC</label><input type="datetime-local" name="tournamentStart" className="tm-input" required value={timeLine.tournamentStart} onChange={handleTimeChange} /></div>
                  <div><label style={{fontSize:9, fontWeight:700, color:'#94a3b8'}}>BẾ MẠC</label><input type="datetime-local" name="tournamentEnd" className="tm-input" required value={timeLine.tournamentEnd} onChange={handleTimeChange} /></div>
                </div>
              </div>
                ...
                {SPORTS_LIST.filter(s => sportsConfig[s].selected).map(s => (
                  <div key={s} className="tm-cat-card">
                    ...
                    <div className="tm-grid">
                      ...
                      <input type="number" className="tm-input" placeholder="Giới hạn số đội" value={sportsConfig[s].maxTeams} onChange={e => handleSportFieldChange(s, 'maxTeams', e.target.value)} />
                    </div>
                    <div style={{ marginTop: 15 }}>
                      <label style={{ fontSize: 10, fontWeight: 700, color: '#64748b' }}>NỘI DUNG:</label>
              ...
              <div className="tm-section">
                ...
                <label style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 15, cursor: 'pointer' }}>
                  ...
                  <span style={{ fontSize: 13, fontWeight: 600, color: '#02457A' }}>Có tổ chức Gala Dinner</span>
                </label>
                {galaConfig.hasGala && (
                  <div className="tm-grid">
                    <input type="datetime-local" name="time" className="tm-input" value={galaConfig.time} onChange={handleGalaChange} />
                    <input name="location" className="tm-input" value={galaConfig.location} onChange={handleGalaChange} placeholder="Địa điểm Gala" />
                    <textarea name="description" className="tm-textarea tm-full" value={galaConfig.description} onChange={handleGalaChange} placeholder="Mô tả Gala..." />
                  </div>
                )}
              </div>
              ...
              <div className="tm-section">
                ...
                <div className="tm-grid">
                  <textarea name="prizes" className="tm-textarea tm-full" value={formData.prizes} onChange={handleTextChange} placeholder="Cơ cấu giải thưởng..." />
