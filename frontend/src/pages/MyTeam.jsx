...
// Mapper để hiển thị tiếng Việt cho hạng mục
...
const MyTeams = () => {
    ...
    const fetchAllData = async () => {
        try {
            ...
            if (invitesRes.data.success) setSentInvites(invitesRes.data.data);
        } catch (err) {
            console.error("Lỗi tải dữ liệu quản lý đội:", err);
        } finally {
            setLoading(false);
        }
    };
    ...
    // 2. LOGIC THU HỒI LỜI MỜI
    const handleCancelInvite = async (id) => {
        if (!window.confirm("Bạn có chắc chắn muốn thu hồi lời mời này?")) return;
        try {
            const res = await api.delete(`/teams/cancel-invitation/${id}`);
            if (res.data.success) {
                setSentInvites(sentInvites.filter(i => i._id !== id));
            }
        } catch (err) {
    ...
    // 3. UI HELPER: TRẠNG THÁI ĐỘI
    const renderStatusBadge = (status) => {
        const styles = {
            'pending_payment': 'bg-red-900/30 text-red-500 border-red-500',
            'active': 'bg-green-900/30 text-green-500 border-green-500',
            'pending_approval': 'bg-yellow-900/30 text-yellow-500 border-yellow-500',
            'Active': 'bg-green-900/30 text-green-500 border-green-500', // Dự phòng cho Enum cũ
        };
        const labels = {
        ...
        return (
            <span className={`px-2 py-1 rounded text-[10px] font-black border ${styles[status] || 'border-gray-500'}`}>
                {labels[status] || status.toUpperCase()}
            </span>
        );
    };

    if (loading) return <div className="text-center py-20 text-cyan-400 font-black animate-pulse">ĐANG TRÍCH XUẤT HỒ SƠ ĐỘI...</div>;
    ...
    return (
        <div className="max-w-6xl mx-auto p-6 animate-fade-in custom-scrollbar">
            ...
            <div className="flex justify-between items-center mb-10 border-b border-cyan-900 pb-6">
                <div>
                    <h1 className="text-3xl font-black text-cyan-400 uppercase tracking-widest drop-shadow-[0_0_8px_rgba(0,240,255,0.4)]">
                        🛡️ TRUNG TÂM ĐIỀU HÀNH ĐỘI
                    </h1>
                    <p className="text-gray-500 text-xs mt-2 uppercase font-bold tracking-widest">Quản lý các nội dung thi đấu bạn đã tham gia</p>
                </div>
                <button 
                    onClick={() => navigate('/register-team')}
                    className="bg-cyan-600 text-white px-6 py-3 rounded-lg font-black text-sm uppercase tracking-widest hover:bg-cyan-500 transition-all shadow-[0_0_15px_rgba(0,240,255,0.3)]"
                >
                    + ĐĂNG KÝ GIẢI MỚI
                </button>
            </div>
                            ...
                            <div key={team._id} className="group relative overflow-hidden bg-slate-900 border border-slate-800 p-6 rounded-2xl hover:border-cyan-500 transition-all shadow-xl">
                                ...
                                <div className="space-y-2 mb-6">
                                    <p className="text-xs text-gray-400 font-bold uppercase">
                                        🏆 Giải: <span className="text-gray-200">{team.tournamentId?.displayName}</span>
                                    </p>
                                    <p className="text-xs text-gray-400 font-bold uppercase">
                                        🏅 Môn: <span className="text-cyan-500">{team.sport}</span> - <span className="text-white">{CATEGORY_MAPPER[team.categoryId] || team.categoryId}</span>
                                    </p>
                                </div>
                                ...
                                <div className="flex gap-2">
                                    ...
                                    {team.status === 'pending_payment' && (
                                        <button 
                                            onClick={() => navigate(`/payment/${team._id}`)} 
                                            className="flex-1 py-3 bg-red-600 text-white text-[10px] font-black uppercase tracking-widest rounded-lg hover:bg-red-500 animate-pulse shadow-[0_0_10px_rgba(220,38,38,0.4)]"
                                        >
                                            Thanh toán ngay
                                        </button>
                                    )}
                            ...
                            <div key={inv._id} className="flex justify-between items-center p-5 bg-slate-900 border border-slate-800 rounded-xl hover:border-red-500/50 transition-all">
                                <div className="flex items-center gap-4">
                                    <div className="w-10 h-10 bg-cyan-900/30 rounded-full flex items-center justify-center text-cyan-400 font-bold">
                                        {inv.userId?.displayName?.charAt(0)}
                                    </div>
                                    <div>
                                        <p className="text-white font-black text-sm uppercase tracking-wide">{inv.userId?.displayName}</p>
                                        <p className="text-[10px] text-gray-500 font-bold uppercase">
                                            Mời vào: {inv.teamId?.teamName} | {inv.teamId?.sport}
                                        </p>
                                    </div>
                                </div>
                                <button 
                                    onClick={() => handleCancelInvite(inv._id)}
                                    className="px-5 py-2 border border-red-500 text-red-500 text-[10px] font-black uppercase tracking-widest rounded hover:bg-red-500 hover:text-white transition-all"
                                >
                                    Thu hồi
                                </button>
                            </div>
