...
const CreateTeamPage = () => {
  ...
  try {
    ...
    // Kiểm tra xem ID có tồn tại thực sự không
    ...
    console.error("Lỗi tạo đội:", error); // Log lỗi chi tiết ra console để debug
    // Hiển thị trực tiếp lỗi từ API trả về thông qua Toast
    ...
    <div className="flex min-h-screen flex-col bg-muted/20">
      ...
      <main className="mx-auto grid w-full max-w-6xl flex-1 gap-6 px-4 py-10 lg:grid-cols-[1fr_340px]">
        <form onSubmit={submit} className="space-y-6 rounded-2xl border border-border bg-card p-6 shadow-sm md:p-8">
          <div>
            <p className="text-xs font-black uppercase tracking-wider text-primary">Dành cho đội trưởng</p>
            <h1 className="mt-2 text-3xl font-black">T?o Đội thi đấu</h1>
            <p className="mt-2 text-sm text-muted-foreground">Chọn giải trước, sau đó nhập thông tin đội. Một player chỉ được tham gia một đội trong cùng giải.</p>
          </div>
        ...
        <aside className="space-y-4">
          <div className="rounded-2xl border border-border bg-card p-5">
            <h2 className="font-black">Sau khi tạo đội</h2>
            <div className="mt-4 space-y-4 text-sm">
              <div className="flex gap-3"><Users className="mt-0.5 h-4 w-4 text-primary" /><span>Đội xuất hiện trong danh sách đội của giải để player tìm và xin gia nhập.</span></div>
          ...
          {selectedTournament && (
            <div className="rounded-2xl bg-primary p-5 text-primary-foreground">
              ...
              <h3 className="mt-2 text-lg font-black">{selectedTournament.name}</h3>
