...
// Helper: kiểm tra user có phải org của giải không
...
export const addCourt = async (req, res) => {
    try {
        ...
        if (!(await isTournamentOrg(userId, tournamentId))) {
            return res.status(403).json({ success: false, message: "Bạn không có quyền thêm sân cho giải này" });
        }

        let sportTypesArray = Array.isArray(sportTypes) ? sportTypes : (sportTypes ? [sportTypes] : ['Pickleball']);
...
export const updateCourt = async (req, res) => {
    try {
        ...
        if (!(await isTournamentOrg(userId, court.tournamentId))) {
            return res.status(403).json({ success: false, message: "Bạn không có quyền cập nhật sân này" });
        }

        if (name) court.name = name;
        if (sportTypes) court.sportTypes = Array.isArray(sportTypes) ? sportTypes : [sportTypes];
